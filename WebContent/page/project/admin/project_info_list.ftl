	...
	<script type="text/javascript">
		...
		$(function(){
		    ...
		    $('.pagination').pagination(${search.totalRecords?c}, {
				...
				link_to: encodeURI('/admin/projectInfo.go?projectType=${search.projectType}&pageNo=__id__')
		    });
	   	});
		...
		<div class="main clear"><!--main开始-->
			...
			<div class="right">
				<div class="m-r">
				    <table class="table2">
		            	<tr>
		                    <th style="width:18%; height:28px;"">项目名称</th>
		                    <th style="width:18%">所属行业</th>
		                    <th style="width:18%">需要融资金额</th>
		                    <th style="width:46%">操作</th>
	               	    </tr>
		               	 <#if list??>
			    			<#list list as item>
					    		<tr>
					    			<td><#if item.projectName??>${item.projectName}</#if></td>
						      		<td><#if item.proindustryId??>${item.proindustryId}</#if></td>
						      		<td><#if item.proPrice??>${item.proPrice}</#if></td>
						      		
						      		<td>
						      			<a href="/admin/projectInfo/detail.go?id=${item.id}">查看详情</a> &nbsp;
						      			<a href="/admin/projectInfo/update.go?id=${item.id}&status=4&projectType=${item.projectType}">下架</a> &nbsp;
						      			<a href="/admin/projectInfo/update.go?id=${item.id}&status=5&projectType=${item.projectType}">成交成功</a> &nbsp;
						      		</td>
						      	</tr>
				      		</#list>
				      	</#if>
